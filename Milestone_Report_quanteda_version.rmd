...
# Set to true to run code chunks that combine and split the raw text files
split_files = TRUE
...
```{r filesize, eval=split_files, message=FALSE, warning=FALSE}
library(stringi)
library(gdata)
...
-   Production data -- A large subset we will use for final learning and
    ...
    ```{r splittingdata, eval=split_files, cache=TRUE, message=FALSE, warning=FALSE, results='hide', echo=TRUE}
    chanceDev <- 0.01
    chanceDevTrain <- 0.7
    chanceProdValid <- 0.5
    chanceProdTrain <- 0.8
